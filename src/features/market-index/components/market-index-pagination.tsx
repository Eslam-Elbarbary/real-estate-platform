...
interface MarketIndexPaginationProps {
  filters: MarketIndexFilters;
  page: number;
  totalPages: number;
}

export function MarketIndexPagination({
  filters,
  page,
  totalPages,
}: MarketIndexPaginationProps) {
  if (totalPages <= 1) return null;

  const hrefFor = (nextPage: number) =>
    buildMarketIndexPath({ ...filters, page: nextPage });

  const pages = Array.from({ length: totalPages }, (_, index) => index + 1);

  return (
    <nav
      aria-label={marketIndexCopy.paginationLabel}
      data-testid="market-index-pagination"
