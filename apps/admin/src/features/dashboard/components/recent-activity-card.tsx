...
const ACTIVITY_ICONS: Record<string, LucideIcon> = {
  'properties-pending': Home,
  'subscriptions-active': CreditCard,
  'leads-new': MessageSquare,
};
    ...
    <Card className={cn(luxuryCardClassName, 'border-0 shadow-none')}>
      <CardHeader className="space-y-1.5 px-6 pt-6 pb-0">
        ...
        <p className="text-sm text-ink-500">
          آخر التحديثات من المراجعة والاشتراكات والعملاء المحتملين
        </p>
      </CardHeader>
      <CardContent className="px-6 pb-6 pt-5">
        <ol className="relative space-y-0">
          {items.map((item, index) => {
            const Icon = ACTIVITY_ICONS[item.id] ?? Clock3;
